...
SAFE coins generated by the use of API's published by the API Network on the SAFE Network will be sent to the API Network Foundation wallet in order purchase APICoins on the Decentralized Exchange. These purchased API Coins (XAP) will be sent on a regular basis to the holders of API Coins (XAP).
